...
if st.button("▶ Compute Per-Start Rows + Probability Summary"):
    ...
    if max_start < 0:
        ...
        for s in range(0, max_start + 1):
            ...
            # Net worth comparison at the horizon: Borrow net is investments minus remaining mortgage
            k_elapsed = months_int
            rem_borrow = remaining_principal(principal=float(principal), annual_rate=float(apr_pct) / 100.0, term_months=term_int, months_elapsed=k_elapsed)
            nw_paycash = bal_cash
        ...
        s1.metric("Pay Cash wins", f"{prob_paycash_wins:.1%}")
        s2.metric("Borrow wins", f"{(1 - prob_paycash_wins):.1%}")
