...
# Contribution timing: contributions are added at the **start** of each month, then the month's return factor applies.
# So: balance = (balance + contrib) * factor
...
def compute_ending_values_first_window(rec_lump: Dict[str, Any],
    ...
    # If a custom payment was passed via closure (attached later), use it; else compute amortized.
    M = getattr(compute_ending_values_first_window, "_custom_payment", None)
    if M is None:
    ...
    # Pay Cash: invest M at the **start** of each month up to the term, then grow by that month's factor
    cash_end = 0.0
    for i in range(horizon_months):
        if i < term_months:
            cash_end += M  # start-of-month contribution
        cash_end *= cf[i]  # then apply the month's return factor
...
with col2:
    apr = st.number_input("Mortgage APR (%)", min_value=0.00, value=5.0, step=0.01) / 100.0
with col3:
...
use_custom = st.checkbox("Use a custom monthly payment amount (override amortized)?", value=False)
if use_custom:
    custom_payment = st.number_input("Custom monthly payment ($)", min_value=0.0, value=2000.0, step=100.0)
else:
    custom_payment = None  # will compute amortized payment
...
if st.button("▶ Compare Ending Portfolio Values"):
    try:
        ...
        # If using a custom monthly payment, attach it to the function as a temporary attribute
        if use_custom:
            compute_ending_values_first_window._custom_payment = float(custom_payment)
        else:
            compute_ending_values_first_window._custom_payment = None
